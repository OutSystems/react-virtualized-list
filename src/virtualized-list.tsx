...
export class VirtualizedList extends React.Component<IVirtualizedListProperties, {}> {
    ...
    private renderItems(startIndex: number, length: number, map: (child: JSX.Element) => JSX.Element) {
        let items: JSX.Element[] = [];
        for (let i = startIndex; i < startIndex + length; i++) {
            items.push(map(this.renderItem(i)));
        }
        return items;
    }
    ...
    public render() {
        ...
        return (
            <VirtualizedScrollViewer renderItems={(start, length, map) => this.renderItems(start, length, map)} 
